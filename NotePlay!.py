...
# schedule = ScheduleOrganizer(mondaySchedules, tuesdaySchedules, wednesdaySchedules, thursdaySchedules, fridaySchedules, saturdaySchedules)
    ...
    mainMenu()
    command = input('Please select the desired number: ')
    ...
    User_SO_Exit = input('\nAre you done making your schedule? Y/N: ')
    if User_SO_Exit == 'Y':
        main()
    else:
        scheduleOrganizer()
...
def notes_menu():
    while True:
        ...
        user_menuInput = input('Please enter the desired number: ')
        if user_menuInput == '1':
...
def input_data():
    ...
    while True: 
        ...
        try:
            x[user_subject_choice][user_inputUNIT][user_inputTERM] = user_inputDESC
        except:
            x[user_subject_choice][user_inputUNIT] = {user_inputTERM: user_inputDESC}

        user_exit = input('Are you done putting datas? (Yes/No): ').upper()
        with open('Subjects.json', 'w') as f:
...
def main_flashcard():
    ...
    '''
    while True:
        print(flashcard_menu)
        choice = input("Please select the desired number: ")
        #Basically the options/choice
        if choice == "1":
            question, answer = create_flashcard()
            flashcards.append((question, answer))
            print("Flashcard created!")
        elif choice == "2":
            if flashcards:
                random_flashcard = random.choice(flashcards)
                flashcard_mode(*random_flashcard)
            else:
...
def main():
    ...
    # HANGMAN
        ...
        while True:
            ...
            print('NOTE: Type "E" to EXIT and return HOME')
            ...
            # i dead the break here because the function main() is unreachable meaning it won't execute
            # if something breaks maybe because of this
            break
    #Quit
    else:
        quit()
