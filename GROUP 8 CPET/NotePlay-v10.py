...
class ScheduleOrganizer:
    ...
    def add_schedule(self) -> str | None:
        while True:
            try:
    ...
    def display_subject_items(self) -> str:
        while True:
            ...
            print(BORDER)
            subject_code: str | int = input("INPUT: ").upper()
            try:
    ...
    def display_time_items(self, title: str) -> Any:
        while True:
            print(BORDER)
            ...
            for num, item in enumerate(iterable=TIME_TUPLE, start=1):
                if item <= 1100 and item >= 700:
                    ante_meridian = int(item * 0.01)
                    print(f"        [{num}] {ante_meridian}:00 AM")
                elif item == 1200:
                    midday = int(item * 0.01)
                    print(f"        [{num}] {midday}:00 PM")
                else:
                    post_meridian = int(item * 0.01) - 12
                    print(f"        [{num}] {post_meridian}:00 PM")
            print(f"\n{BORDER}")
            print("TO QUIT TYPE 'QUIT'")
            print(BORDER)
            if title == "START":
                subject_start_time: str | int = input("INPUT: ")
                try:
                ...
                subject_end_time: str | int = input("INPUT: ")
                try:
    ...
    def display_day_items(self) -> Any:
        while True:
            print(BORDER)
            ...
            print("TO QUIT TYPE 'QUIT'")
            print(BORDER)
            subject_day: str | int = input("INPUT: ")
            try:
        ...
        while True:
            ...
            if view_schedule_select == 1:
                ...
                for day, subjs in schedules_json_data.items():
                    ...
                    for subj, attrs in subjs.items():
                        ...
                        for attr, val in attrs.items():
                            Schedule_Organizer.convert_time(key=attr, time=val)
                    print(f"\n{BORDER}")
                ...
                while True:
                    ...
                    print(f"{BORDER}")
                    schedule_select: str | int = input("INPUT: ").upper()
                    try:
                        schedule_select = int(schedule_select)
                        if schedule_select > len(DAY_TUPLE) or schedule_select < 0:
                            continue
                        else:
                            clr_terminal()
                            print(f"{BORDER}")
    ...
    def convert_time(self, key, time):
        if type(time) == int:
            if time <= 1100 and time >= 700:
                ante_meridian = int(time * 0.01)
                print(f"        {key}: {ante_meridian}:00 AM")
            elif time == 1200:
                midday = int(time * 0.01)
                print(f"        {key}: {midday}:00 PM")
            elif time > 1200:
                post_meridian = int(time * 0.01) - 12
                print(f"        {key}: {post_meridian}:00 PM")
        else:
            print(f"        {key}: {time}")
...
class Notes:
    ...
    def add_notes(self) -> None | str:
        while True:
            ...
            try:
                ...
                if notes_definition == "1":
                    return "1"
                elif notes_definition == '2':
                    continue
                else:
                    pass
            except:
    ...
    def get_user_input_notes(self, title: str) -> Any:
        clr_terminal()
        print(f"{BORDER}\nENTER A {title}\n{BORDER}")
        if title == "SUBJECT":
            print("")
            ...
            print("TO QUIT TYPE 'QUIT'")
            print(f"{BORDER}")
            user_input_int: str | int = input("INPUT: ").upper()
            try:
...
class Hangman:
    ...
    def get_user_input_hangman_settings(self) -> int | str:
        '''1: set hangman rounds, 2: quit hangman settings'''
        while True:
            user_input: str | int = input("INPUT: ")
            try:
    ...
    def set_hangman_rounds(self):
        while True:
            ...
            rounds: str | int = input("INPUT: ").upper()
            try:
                rounds = int(rounds)
                return rounds
            except:
            ...
            while True:
                ...
                if hangman_input == 1: # PLAY HANGMAN
                    ...
                    while True:
                        ...
                        print(f"\n{BORDER}")
                        hangman_settings_menu_input: int | str = Hangman_.get_user_input_hangman_settings()
                        if hangman_settings_menu_input == 1:
                            user_input_code = Hangman_.set_hangman_rounds()
                            if user_input_code == "1":
                                break
                            hangman_rounds = user_input_code
                            
                        elif hangman_settings_menu_input == 2:
                            Hangman_.quit_hangman_settings()
                        
                        elif hangman_settings_menu_input == "1":
                            break
